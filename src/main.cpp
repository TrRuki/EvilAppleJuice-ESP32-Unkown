...
#define ONLY_UNKNOWN_DEVICES 1 // Set to 1 to only use unknown devices, 0 to use all devices
...
void loop() {
  ...
  #if ONLY_UNKNOWN_DEVICES
  int device_choice = random(1);
  #else
  ...
  if (device_choice == 0){
    int index = random(2);
      ...
      #else
          oAdvertisementData.addData(std::string((char*)UNKNOWN_DEVICES[index], 31));
      #endif
    #endif
